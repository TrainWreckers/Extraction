...
class TW_Global 
{
	...
	static void GetAllAddons(out notnull array<ref SCR_WorkshopItem> addons)
	{
		SCR_AddonManager instance = SCR_AddonManager.GetInstance();
		
		if(!instance)
			return;
		
		addons = instance.GetAllAddons();		
	}
