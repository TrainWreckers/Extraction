...
private static ref map<SCR_EArsenalItemType, ref array<ref SCR_TW_LootItem>> s_Items = new map<SCR_EArsenalItemType, ref array<ref SCR_TW_LootItem>>();	
  ...
  static void SInitializeCatalog()
  {
    ...
    foreach(SCR_EntityCatalogEntry entry : catalogItems)
      ...
      foreach(SCR_BaseEntityCatalogData data : itemData)
      {
        ...
        ref SCR_TW_LootItem item = new SCR_TW_LootItem(entry.GetEntityName(), itemType, itemMode, prefab);
  ...
  static void GetItemsForFlag(int type, out notnull array<ref SCR_TW_LootItem> items)
  {
    ...
    static int GetItemsByType(SCR_EArsenalItemType itemFlags, SCR_EArsenalItemMode itemModes, out notnull array<ref SCR_TW_LootItem> items)	
    {
      int count = 0;
      ...
      if(itemFlags == 0)
		{
        foreach(auto type, auto lootItems : s_Items)
                 {
          foreach(auto item : lootItems)
            if(item)
              items.Insert(item);
					...
					{
              Print("TrainWreck: There shouldn't be null values in loot items", LogLevel.ERROR);
              break;						
            }
        }
      }
    ...
    override void OnPostInit(IEntity owner)
	{
      ...
      ref array<ref SCR_TW_LootItem> entries = {};
    ...
    ref array<ref SCR_TW_LootItem> entries = {};
    int count = SCR_TW_ExtractionHandler.GetInstance().GetItemsByItemFlags(m_lootItemTypes, entries);
